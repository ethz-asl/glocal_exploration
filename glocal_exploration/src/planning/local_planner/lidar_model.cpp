...
bool LidarModel::setupFromConfig(SensorModel::Config *config) {
  ...
  if (!cfg) {
    LOG(ERROR) << "Failed to setup: config is not of type 'LidarModel::Config'.";
    return false;
  }
  config_ = *cfg;

  // params
  CHECK_GT(config_.vertical_fov, 0) << "The vertical field of view is expected > 0";
  CHECK_GT(config_.horizontal_fov, 0) << "The horizontal field of view is expected > 0";
  CHECK_GT(config_.vertical_resolution, 0) << "The vertical resolution is expected > 0";
  CHECK_GT(config_.horizontal_resolution, 0) << "The horizontal resolution is expected > 0";
...
bool LidarModel::getVisibleVoxels(std::vector<Eigen::Vector3d> *result, const WayPoint &waypoint) {
...
void LidarModel::getDirectionVector(Eigen::Vector3d *result, double relative_x, double relative_y) {
  double phi = (0.5 - relative_x) * c_fov_x_;
  double theta = M_PI / 2.0 + (relative_y - 0.5) * c_fov_y_;
  *result = Eigen::Vector3d(sin(theta) * cos(phi), sin(theta) * sin(phi), cos(theta));
}
